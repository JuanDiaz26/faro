...
async function create(req, res) {
  ...
  res.status(201).json(await db.prepare('SELECT * FROM life_goals WHERE id = ?').get(lastInsertRowid))
}
...
async function update(req, res) {
  ...
  res.json(await db.prepare('SELECT * FROM life_goals WHERE id = ?').get(id))
}

// DELETE /api/life-goals/:id
async function remove(req, res) {
  const { changes } = await db.prepare('DELETE FROM life_goals WHERE id = ?').run(req.params.id)
  if (changes === 0) return res.status(404).json({ error: 'Meta no encontrada' })
  res.status(204).send()
}
